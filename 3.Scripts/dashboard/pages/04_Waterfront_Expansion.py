# Q2 — Waterfront Expansion Analysis
...
def load_data():
    wf = pd.read_csv(csv_path("waterfront_stations.csv"))
    hot = pd.read_csv(csv_path("waterfront_hotspot_hourly_summary.csv"))
    return wf, hot

try:
    with st.spinner("Loading pre-computed analysis data…"):
        wf, hotspot_hourly_summary = load_data()
    st.success("Using pre-computed files. For the full workflow, see NYC_Q2_Waterfront_Expansion.ipynb.")
except FileNotFoundError as e:
    st.error(f"File not found in repo: {e}")
    st.stop()
...
c1.metric("Waterfront Stations Share", f"{supply_share:.1%}")
c2.metric("Waterfront Endpoints Share", f"{demand_share:.1%}")
c3.metric("Station Gap", f"{shortfall}")
...
wf = wf.sort_values("share_of_water_endpoints", ascending=False).reset_index(drop=True)
...
fig.update_layout(
    ...
    showlegend=False,
    xaxis=dict(range=[0, 1], tickformat=".0%", tickmode="array",
               tickvals=[0, 0.5, 1.0], ticktext=["0%", "50%", "100%"], title=None),
